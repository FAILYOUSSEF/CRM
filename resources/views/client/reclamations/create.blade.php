...
<div class="bg-white rounded-lg shadow p-6 max-w-xl">
    <form method="POST" action="{{ route('client.reclamations.store') }}" class="space-y-4">
        ...
        <div><label class="block text-sm font-medium text-gray-800 mb-1">Type</label>
            <input name="type" value="{{ old('type') }}" class="w-full border border-gray-400 rounded px-3 py-2 text-gray-900 placeholder-gray-500 bg-white"></div>
        ...
        <div class="flex gap-3">
            ...
            <a href="{{ route('client.reclamations.index') }}" class="text-gray-600 hover:underline text-sm py-2">Cancel</a>
        </div>
    </form>
</div>
@endsection