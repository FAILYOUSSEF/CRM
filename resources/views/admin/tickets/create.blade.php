...
<div class="max-w-2xl mx-auto bg-white p-8 rounded shadow">
    <h1 class="text-2xl font-bold mb-6 text-gray-900">Create New Ticket</h1>
    <form method="POST" action="{{ route('admin.tickets.store') }}">
        @csrf
        <div class="mb-4">
            <label for="sujet" class="block mb-1 font-semibold text-gray-800">Subject</label>
            <input type="text" name="sujet" id="sujet" class="w-full border border-gray-400 rounded px-3 py-2 text-gray-900 placeholder-gray-500 bg-white" required value="{{ old('sujet') }}">
        ...
        <div class="mb-4">
            ...
            <select name="priorite" id="priorite" class="w-full border border-gray-400 rounded px-3 py-2 text-gray-900 bg-white" required>
                <option value="faible" {{ old('priorite') == 'faible' ? 'selected' : '' }}>Low</option>
                <option value="moyen" {{ old('priorite') == 'moyen' ? 'selected' : '' }}>Medium</option>
                <option value="urgent" {{ old('priorite') == 'urgent' ? 'selected' : '' }}>High</option>
            </select>
        ...
        <div class="mb-4">
            ...
            <textarea name="message" id="message" rows="4" class="w-full border border-gray-400 rounded px-3 py-2 text-gray-900 placeholder-gray-500 bg-white">{{ old('message') }}</textarea>
        ...
        <div class="mb-4">
            ...
            <input type="date" name="deadline" id="deadline" class="w-full border border-gray-400 rounded px-3 py-2 text-gray-900 bg-white" value="{{ old('deadline') }}">
        ...
        <div class="mb-4">
            ...
            <select name="status" id="status" class="w-full border border-gray-400 rounded px-3 py-2 text-gray-900 bg-white" required>
                <option value="ouvert" {{ old('status') == 'ouvert' ? 'selected' : '' }}>Open</option>
                <option value="fermé" {{ old('status') == 'fermé' ? 'selected' : '' }}>Closed</option>
            </select>
