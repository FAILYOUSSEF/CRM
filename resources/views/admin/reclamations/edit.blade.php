...
@section('content')
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-crm-text tracking-tight">Edit Reclamation</h1>
    </div>

    <div class="bg-crm-surface border border-crm-border rounded-crm shadow-sm p-6 sm:p-8 max-w-2xl">
        <form method="POST" action="{{ route('admin.reclamations.update', $reclamation->id) }}" class="space-y-6">
        @csrf
        @method('PUT')
        <div>
            <label for="titre" class="block text-sm font-medium text-crm-muted mb-1">Title</label>
            <input type="text" name="titre" id="titre" class="block w-full bg-crm-bg3 border border-crm-border rounded-crm text-crm-text placeholder-crm-muted/50 focus:ring-2 focus:ring-crm-accent focus:border-crm-accent transition-all duration-200 sm:text-sm py-2.5" required value="{{ old('titre', $reclamation->titre) }}">
        ...
        <div>
            ...
            <select name="type" id="type" class="block w-full bg-crm-bg3 border border-crm-border rounded-crm text-crm-text focus:ring-2 focus:ring-crm-accent focus:border-crm-accent transition-all duration-200 sm:text-sm py-2.5" required onchange="handleTypeChange()">
                ...
                <option value="billing" @selected(old('type', $reclamation->type) == 'billing')>Billing</option>
                <option value="product" @selected(old('type', $reclamation->type) == 'product')>Product/Service</option>
                <option value="delivery" @selected(old('type', $reclamation->type) == 'delivery')>Delivery</option>
                <option value="quality" @selected(old('type', $reclamation->type) == 'quality')>Quality</option>
                <option value="other" @selected(old('type', $reclamation->type) == 'other')>Other</option>
            </select>
            @error('type') <div class="text-red-500 text-xs mt-1">{{ $message }}</div> @enderror
        </div>

        <div id="type_other_div" style="display: {{ old('type', $reclamation->type) == 'other' ? 'block' : 'none' }};">
            <label for="type_other" class="block text-sm font-medium text-crm-muted mb-1">Other Type (Please specify)</label>
            <input type="text" name="type_other" id="type_other" class="block w-full bg-crm-bg3 border border-crm-border rounded-crm text-crm-text placeholder-crm-muted/50 focus:ring-2 focus:ring-crm-accent focus:border-crm-accent transition-all duration-200 sm:text-sm py-2.5" value="{{ old('type_other', $reclamation->type_other) }}">
        ...
        <div>
            ...
            <select name="priorite" id="priorite" class="block w-full bg-crm-bg3 border border-crm-border rounded-crm text-crm-text focus:ring-2 focus:ring-crm-accent focus:border-crm-accent transition-all duration-200 sm:text-sm py-2.5" required>
                <option value="faible" @selected(old('priorite', $reclamation->priorite) == 'faible')>Low</option>
                <option value="moyenne" @selected(old('priorite', $reclamation->priorite) == 'moyenne')>Medium</option>
                <option value="haute" @selected(old('priorite', $reclamation->priorite) == 'haute')>High</option>
                <option value="critique" @selected(old('priorite', $reclamation->priorite) == 'critique')>Critical</option>
            </select>
        ...
        <div>
            ...
            <textarea name="description" id="description" rows="4" class="block w-full bg-crm-bg3 border border-crm-border rounded-crm text-crm-text placeholder-crm-muted/50 focus:ring-2 focus:ring-crm-accent focus:border-crm-accent transition-all duration-200 sm:text-sm py-2.5">{{ old('description', $reclamation->description) }}</textarea>
        ...
        <div>
            ...
            <select name="assigned_to" id="assigned_to" class="block w-full bg-crm-bg3 border border-crm-border rounded-crm text-crm-text focus:ring-2 focus:ring-crm-accent focus:border-crm-accent transition-all duration-200 sm:text-sm py-2.5">
                ...
                @foreach($users as $user)
                    <option value="{{ $user->id }}" @selected(old('assigned_to', $reclamation->assigned_to) == $user->id)>{{ $user->name }}</option>
                @endforeach
            </select>
        ...
        <div>
            ...
            <select name="status" id="status" class="block w-full bg-crm-bg3 border border-crm-border rounded-crm text-crm-text focus:ring-2 focus:ring-crm-accent focus:border-crm-accent transition-all duration-200 sm:text-sm py-2.5" required>
                <option value="en attente" @selected(old('status', $reclamation->status) == 'en attente')>Pending</option>
                <option value="traité" @selected(old('status', $reclamation->status) == 'traité')>Treated</option>
                <option value="resolu" @selected(old('status', $reclamation->status) == 'resolu')>Resolved</option>
            </select>
